Plataforma de estudos personalizável para ENEM e vestibulares. O projeto oferece um ambiente de estudos customizável, permitindo que o usuário adicione seu próprio banco de questões.

O banco de questões deve ser composto por questões de múltipla escolha, contendo obrigatoriamente:

- Enunciado da questão
- Alternativas
- Alternativa correta
- A plataforma importa o arquivo enviado, organiza o conteúdo em blocos de questões para facilitar a visualização e permite que os usuários acessem e resolvam as questões

Este é um projeto pessoal de estudos desenvolvido em Django, atualmente em desenvolvimento. Estou constantemente implementando novas funcionalidades e corrigindo falhas.

Na raiz do projeto existe o arquivo todo.md que me auxilia a acompanhar o progresso do desenvolvimento, nele registro:

- Alterações realizadas
- Pontos de melhoria
- Bugs identificados
